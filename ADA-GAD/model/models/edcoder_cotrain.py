...
class PreModelCotrain(nn.Module):
    ...
    def forward(self, x, edge_index, mask_type):
        ...
        if loss is None:
            print("[edcoder_cotrain] forward: loss为None，直接返回None。"); return None, None, None
        loss_item = {"loss": loss.item()}
        return loss, loss_item,final_edge_mask_rate

    def intersection_edge(self,edge_index_1, edge_index_2,max_num_nodes): 
        if max_num_nodes > 10000:
            print(f"[edcoder_cotrain] intersection_edge: 节点数为{max_num_nodes}，跳过to_dense_adj，返回None！")
            return None, None
        # s1=to_dense_adj(edge_index_1,max_num_nodes=max_num_nodes)[0]
        # s2=to_dense_adj(edge_index_2,max_num_nodes=max_num_nodes)[0]
        intersection_s=torch.min(edge_index_1.to(torch.float32),edge_index_2.to(torch.float32))

        intersection_edge_index,_=remove_self_loops(add_self_loops(intersection_s)[0])
    ...
    def mask_attr_prediction(self, x, edge_index,mask_type):
        ...
        if num_nodes > 10000:
            print(f"[edcoder_cotrain] mask_attr_prediction: 节点数为{num_nodes}，跳过to_dense_adj相关操作，返回None！")
            return None, None
        ...
        if _drop_edge_rate>0 or _drop_path_rate>0 :
            ...
            s_init = edge_index.to(torch.float32)
